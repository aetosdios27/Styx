...
//!
//! Phase 2 starts with BEP 3 bencode encoding and decoding. The parser treats
//! all input bytes as untrusted and returns typed errors for malformed data.
...
pub enum BencodeError {
    ...
    /// Torrent metainfo parsing has not been implemented yet.
...
impl fmt::Display for BencodeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TorrentMetainfoNotImplemented => {
                write!(f, "torrent metainfo parsing is not implemented yet")
            }
        }
    }
}
...
/// Placeholder for typed torrent metainfo parsing.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// Decode a `.torrent` metainfo file.
///
/// # Errors
///
/// Always returns [`BencodeError::TorrentMetainfoNotImplemented`] until typed
/// metainfo parsing is implemented after the core bencode layer.
...
impl Parser<'_> {
    fn parse_value(&mut self, depth: usize) -> Result<BencodeValue, BencodeError> {
    ...
    fn parse_integer(&mut self) -> Result<BencodeValue, BencodeError> {
        ...
        Ok(BencodeValue::Integer(integer))
    }
    ...
    fn parse_list(&mut self, depth: usize) -> Result<BencodeValue, BencodeError> {
        self.offset += 1;
        ...
        loop {
            match self.peek() {
                Some(b'e') => {
                    self.offset += 1;
                    return Ok(BencodeValue::List(values));
                }
    ...
    fn parse_dict(&mut self, depth: usize) -> Result<BencodeValue, BencodeError> {
        self.offset += 1;
        ...
        loop {
            match self.peek() {
                Some(b'e') => {
                    self.offset += 1;
                    return Ok(BencodeValue::Dict(values));
                }
                Some(b'0'..=b'9') => {
...
mod tests {
    ...
    #[test]
    fn decode_torrent_is_explicitly_not_implemented() {
        assert_eq!(
            decode_torrent(b"d4:infodee"),
            Err(BencodeError::TorrentMetainfoNotImplemented)
        );
    }
